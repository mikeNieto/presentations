...
class SourceCode extends BaseHTMLElement {
    ...
    async showCurrentTab(current) {
        ...
        this.$$("a").forEach((link, index) =>
            link.classList.toggle("selected", index == current)
        );
    }
}
...
class WebTerm extends BaseHTMLElement {
    ...
    addTab() {
        ...
        window
            ...
            .addEventListener("click", () => this.addTab());
    }
}
...
class NavArrows extends BaseHTMLElement {
    ...
    render() {
        ...
        }" onclick="window.location.href='${this.previous}';">&lt;</a>
        <a class="${
            this.next ? "" : "disabled"
        }" onclick="window.location.href='${this.next}';">&gt;</a>`;
    }
